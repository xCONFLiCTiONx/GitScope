...
async function getOrStartServer(projectRoot) {
  ...
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      if (ext === '.html' || ext === '.htm') {
        ...
        stream.on('error', (err) => {
          logError(`Stream error serving file "${fileOnDisk}": ${err.message}`, err);
          if (!res.headersSent) {
  ...
  try {
    ...
    watcher.on('error', (err) => {
      logError(`File watcher error for root "${normalizedRoot}": ${err.message}`, err);
    });

    const triggerReload = () => {
      if (debounceTimer) clearTimeout(debounceTimer);
      debounceTimer = setTimeout(() => {
        for (const clientRes of Array.from(clientsSet)) {
          try {
            clientRes.write('data: reload\n\n');
          } catch (e) {
            logError(`Live reload write error: ${e.message}`);
    ...
    logError(`Failed to initialize file watcher for root "${normalizedRoot}": ${err.message}`, err);
  }
