    ...
    try {
      if (process.platform === 'win32') {
        try {
          ...
          execSync(`attrib -r -h -s "${lockFile}"`, { stdio: 'ignore' });
        } catch (e) {}
      }
  ...
  try {
    ...
    if (await fs.pathExists(gitDir)) {
      if (process.platform === 'win32') {
        try {
          const { execSync } = require('child_process');
          execSync(`attrib -r -h -s "${gitDir}\\*.*" /s /d`, { stdio: 'ignore' });
        } catch (e) {}
      }
      try {
        ...
        if (process.platform === 'win32') {
          const { execSync } = require('child_process');
          execSync(`cmd.exe /c rmdir /s /q "${gitDir}"`, { stdio: 'ignore' });
        }
      }
    }
  ...
  try {
    ...
    switch (level) {
      ...
      case 'local-clean':
        ...
        break;
      case 'force-push':
        const pushAll = await git.raw(['push', 'origin', '--force', '--all']);
        const pushTags = await git.raw(['push', 'origin', '--force', '--tags']);
        output = `Force Push All: ${pushAll || 'Success'}\nForce Push Tags: ${pushTags || 'Success'}`;
        break;
      default:
