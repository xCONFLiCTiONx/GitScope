...
async function push(repoPath) {
  ...
  try {
    ...
    const res = await git.raw(['push', '-u', 'origin', status.current]);
...
async function getQuickDashboardStatus(repoPath) {
  try {
    ...
    // Intelligence: Use a specialized git instance with interactive prompts disabled
    // to prevent background dashboard refreshes from nagging the user with login windows.
    ...
    // 1. Fetch tracking info for speed (User requested fetch origin --quiet --depth=1)
    try {
      await git.raw(['fetch', 'origin', '--quiet', '--depth=1']);
    } catch (e) {
      try {
        await git.raw(['fetch', '--quiet', '--depth=1']);
      } catch (e2) {}
    }

    // 2. Get status. ahead/behind are calculated against the fetched tracking branch.
    // We also use rev-list as a fallback because status sometimes fails to report ahead/behind
    // in shallow clones or specific git configurations.
    ...
    // Intelligence: Fallback ahead/behind check using rev-list if status reports 0
    // but a tracking branch exists.
    if (ahead === 0 && behind === 0 && statusResult.current) {
        try {
            const tracking = await git.raw(['rev-parse', '--abbrev-ref', `${statusResult.current}@{u}`]);
            if (tracking.trim()) {
                const aheadRaw = await git.raw(['rev-list', '--count', `${tracking.trim()}..HEAD`]);
                const behindRaw = await git.raw(['rev-list', '--count', `HEAD..${tracking.trim()}`]);
...
async function subtreePush(repoPath, prefix, remoteUrl, branch = 'main', force = false) {
  ...
  try {
    ...
    if (force) {
      ...
      const res = await git.raw(['push', remoteUrl, `${splitHash.trim()}:refs/heads/${branch}`, '--force']);
      return { success: true, output: res };
    } else {
      // Standard Subtree Push
      const res = await git.raw(['subtree', 'push', `--prefix=${prefix}`, remoteUrl, branch]);
      return { success: true, output: res };
    }
  } catch (e) {
    return { success: false, output: e.message };
...
module.exports = { getStatus, getRawStatus, pull, fetch, push, commit, getBranches, switchBranch, getFullDiff, getStagedDiff, getChangedFiles, getDetailedChanges, getQuickDashboardStatus, getFileDiff, getFilePatch, applyPatch, getCommits, revertToCommit, restoreToHead, restoreFile, initRepository, nukeAndReinit, createBranch, deleteBranch, renameBranch, stageAllChanges, unstageAllChanges, getRemotes, addRemote, removeRemote, stageFile, unstageFile, publishSequence, stashSave, stashList, stashPop, stashApply, stashDrop, stopTracking, startTracking, isTracked, subtreePush };
