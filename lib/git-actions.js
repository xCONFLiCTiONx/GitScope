...
async function getStatus(repoPath) {
  try {
    ...
    const details = await getDetailedChanges(repoPath);
...
async function getDetailedChanges(repoPath) {
  try {
    ...
    // Use git.raw for faster/more exhaustive status info including ignored
    const statusRaw = await git.raw(['status', '--porcelain', '--ignored']);
...
async function stageAllChanges(repoPath) {
  ...
  try {
    ...
    if (errorMsg.includes('Permission denied') || errorMsg.includes('locked')) {
      console.warn('Backend: Full stage failed due to locked files. Attempting partial stage of tracked files...');
      try {
        ...
        return {
          success: true,
          partial: true,
          output: 'Partial Stage: Staged all tracked changes. Some untracked files are locked by another process (e.g., Visual Studio) and were skipped.'
        };
      } catch (e2) {
        return { success: false, output: e.message };
      }
...
module.exports = { getStatus, getRawStatus, pull, fetch, merge, push, commit, getBranches, switchBranch, getFullDiff, getStagedDiff, getChangedFiles, getDetailedChanges, getQuickDashboardStatus, getFileDiff, getFilePatch, applyPatch, getCommits, revertToCommit, restoreToHead, restoreFile, initRepository, nukeAndReinit, createBranch, deleteBranch, renameBranch, stageAllChanges, unstageAllChanges, getRemotes, addRemote, removeRemote, setRemoteUrl, stageFile, unstageFile, publishSequence, stashSave, stashList, stashPop, stashApply, stashDrop, stopTracking, startTracking, isTracked, subtreePush, subtreePull, getCommitDiff, addToGitignore };
