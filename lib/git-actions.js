...
async function getFileDiff(repoPath, filePath) {
  ...
  try {
    ...
    let diff = '';
    if (hasCommits) {
        // Standard diff against HEAD
        diff = await git.diff(['HEAD', '--', filePath]);
    } else {
        // For unborn repos, we show the file as a new file (all additions)
        // We can use git diff / dev / null to trick it or just read the file
        // However, a simple git diff --cached (for staged) or just nothing works
        // But the most robust way to show the "entire file as added" for unborn is:
        try {
            // First check if it's staged
            const staged = await git.diff(['--cached', '--', filePath]);
            // Then check if it's unstaged
            const unstaged = await git.diff(['--', filePath]);

            // If it's a completely new project, git diff won't show anything for untracked files
            // So we handle untracked files by manually constructing an "add" diff if needed
            if (!staged && !unstaged) {
                const fs = require('fs-extra');
...
module.exports = { getStatus, getRawStatus, pull, fetch, push, commit, getBranches, switchBranch, getFullDiff, getStagedDiff, getChangedFiles, getDetailedChanges, getFileDiff, getFilePatch, applyPatch, getCommits, revertToCommit, restoreToHead, restoreFile, initRepository, nukeAndReinit, createBranch, deleteBranch, renameBranch, stageAllChanges, unstageAllChanges, getRemotes, addRemote, removeRemote, stageFile, unstageFile, publishSequence, stashSave, stashList, stashPop, stashApply, stashDrop, stopTracking, startTracking, isTracked };
