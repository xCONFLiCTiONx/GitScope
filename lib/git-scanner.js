...
const IGNORE_DIRS = ['.git', 'node_modules', 'dist', 'build', '.vs', '.idea'];
  ...
  if (!showIgnored) {
    try {
      ...
      if (gitRoot) {
        ...
        const relativePaths = result.map(item => path.relative(gitRoot, item.path));

        // Use check-ignore to find which files ARE ignored
        const ignoredPathsSet = new Set();
        try {
          // checkIgnore can take an array of paths
          const ignored = await git.checkIgnore(relativePaths);
          ignored.forEach(p => ignoredPathsSet.add(path.resolve(gitRoot, p).replace(/\\/g, '/').toLowerCase()));
        } catch (e) {
