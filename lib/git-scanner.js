...
async function scanDirectory(rootPath) {
  ...
  for (const item of items) {
    ...
    const winAttrs = getWindowsAttributes(fullPath);
...
async function listDirectory(dirPath, viewModeOrShowIgnored = true) {
  ...
  for (const item of items) {
    ...
    // 2. Skip hardcoded ignore dirs if viewMode is not 'all'.
    if (viewMode !== 'all' && IGNORE_DIRS.includes(item)) continue;
...
module.exports = { scanDirectory, listDirectory, naturalSort, getUnbornFolders, getWindowsAttributes };
