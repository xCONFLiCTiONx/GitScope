...
async function scanDirectory(rootPath) {
  ...
  for (const item of items) {
    if (item === '.git') continue; // Don't scan inside .git folders for other repos
...
async function listDirectory(dirPath, showIgnored = true) {
  ...
  for (const item of items) {
    // Only skip hardcoded system/ignore dirs if showIgnored is false.
    // When showIgnored is true, we want to see everything (all folders and files).
