...
if (!gotTheLock) {
  ...
  async function detectInstalledEditors() {
    ...
    try {
      ...
      if (fs.existsSync(vswhere)) {
        const output = execSync(`"${vswhere}" -latest -property productPath`, {
          encoding: 'utf8',
        }).trim();
  ...
  ipcMain.handle('ensure-github-ssh-trust', async () => {
    try {
      ...
      const keys = execSync('ssh-keyscan -t ed25519,rsa github.com', {
        encoding: 'utf8',
        timeout: 5000,
      });
  ...
  ipcMain.handle('git-sync-token', async (event, token) => {
    try {
      ...
      if (res.status === 0) {
        return {
          success: true,
          message: `Successfully injected token for ${username}@github.com into your system credential helper.`,
        };
      } else {
  ...
  ipcMain.handle('git-clear-creds', async () => {
    try {
      ...
      if (res.status === 0) {
        return {
          success: true,
          message: 'Successfully cleared GitHub credentials from your system helper.',
        };
      } else {
        return { success: false, error: res.stderr || 'Git credential command failed' };
      }
  ...
  ipcMain.handle('generate-commit-msg', async (event, diff) => {
    ...
    else if (foundKeywords.includes('refactor') || (additions > 0 && additions === deletions))
    ...
    const styles = [
      () => {
        ...
        if (fileList.length === 1) summary = `modify ${path.basename(fileList[0])}`;
          ...
          summary =
            dirList.length === 1
              ? `update components in ${dirList[0]}`
              : `update ${fileList.length} files`;
        }
...
`;
  ...
  ipcMain.handle('save-git-config', async (event, content) => {
    ...
    return {
      success: false,
      error: lastError ? `${lastError.code}: ${lastError.message}` : 'Access Denied',
    };
  });
  ...
  ipcMain.handle('import-settings', async () => {
    ...
    try {
      ...
      if (backup.repositories)
        await fs.writeJson(configPath, { repositories: backup.repositories });
  ...
  ipcMain.handle('open-in-visual-studio', async (event, filePath) => {
    ...
    if (fs.existsSync(vswhere)) {
      try {
        const installPath = require('child_process')
          .execSync(`"${vswhere}" -latest -property installationPath`, { encoding: 'utf8' })
          .trim();
  ...
  ipcMain.handle(
    'open-content-in-chrome',
    async (event, { content, filename, identifier, skipOpen }) => {
      try {
    ...
    exec(
      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          reportError(
            'Execution Failed',
            `Could not execute ${path.basename(filePath)} as administrator: ${error.message}`,
          );
        }
      },
    );
        ...
        submenu: [
          {
            label: 'File',
            click: () =>
              event.sender.send('context-menu-command', { command: 'new-file', path: paths[0] }),
          },
          {
            label: 'Folder',
            click: () =>
              event.sender.send('context-menu-command', { command: 'new-folder', path: paths[0] }),
          },
        ],
      });
      template.push({ type: 'separator' });
      ...
      if (!isMulti) {
        if (!isFolder) {
          template.push({
            label: 'See Changes',
            click: () =>
              event.sender.send('context-menu-command', { command: 'see-changes', path: paths[0] }),
          });
        }
        ...
        template.push({
          label: 'Rename',
          click: () =>
            event.sender.send('context-menu-command', { command: 'rename', path: paths[0] }),
        });
      }
      template.push({
      ...
      template.push({
        ...
        submenu: [
          ...
          },
          {
            label: 'Add LICENSE',
            click: () =>
              event.sender.send('context-menu-command', { command: 'add-license', path: paths[0] }),
          },
        ],
      });
      template.push({ type: 'separator' });
